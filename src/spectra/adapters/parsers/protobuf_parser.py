"""
Protobuf Parser - Parse Protocol Buffer files into domain entities.

Implements the DocumentParserPort interface for .proto files.

Extracts user stories and requirements from Protobuf comments and structure.
Useful for API-driven specifications where Protobuf defines the contracts.
"""
...
class ProtobufParser(DocumentParserPort):
    """
    Parser for Protocol Buffer (.proto) specification files.

    Extracts stories from structured comments in proto files.
    ...
    def __init__(self) -> None:
        ...
        self.logger = logging.getLogger("ProtobufParser")
    ...
    def can_parse(self, source: str | Path) -> bool:
        """Check if source is a valid proto file or content."""
        if isinstance(source, Path):
            return source.suffix.lower() in self.supported_extensions

        # Check for proto-like content
        has_syntax = bool(re.search(r'syntax\s*=\s*"proto[23]"', source))
        has_message = bool(re.search(r"message\s+\w+\s*\{", source))
        has_service = bool(re.search(r"service\s+\w+\s*\{", source))

        return has_syntax or has_message or has_service

    def parse_stories(self, source: str | Path) -> list[UserStory]:
        """Parse user stories from proto source."""
        content = self._get_content(source)
        return self._parse_all_stories(content)

    def parse_epic(self, source: str | Path) -> Epic | None:
        """Parse an epic from proto source."""
        content = self._get_content(source)
        ...
        if epic_title == "Untitled Epic":
            ...
            if pkg_match:
                epic_title = pkg_match.group(1).replace(".", " ").title()
    ...
    def validate(self, source: str | Path) -> list[str]:
        """Validate proto source without full parsing."""
        errors: list[str] = []
        ...
        if not re.search(r'syntax\s*=\s*"proto[23]"', content):
            errors.append("Missing proto syntax declaration")

        # Check for story comments
        story_pattern = rf"//\s*({self.STORY_ID_PATTERN}):\s*"
        if not re.search(story_pattern, content):
            errors.append("No user stories found in comments")
    ...
    def _parse_all_stories(self, content: str) -> list[UserStory]:
        """Parse all stories from proto comments."""
        stories = []

        # Find comment blocks that define stories
        story_pattern = r"((?://[^\n]*\n)+)(?=\s*(?:message|service|rpc|enum)\s+)"
        for match in re.finditer(story_pattern, content):
            comment_block = match.group(1)
            story = self._parse_story_from_comment(comment_block)
            if story:
                stories.append(story)

        return stories

    def _parse_story_from_comment(self, comment_block: str) -> UserStory | None:
        """Parse a story from a comment block."""
        # Extract story ID and title from first line
        header_match = re.search(rf"//\s*({self.STORY_ID_PATTERN}):\s*([^\n]+)", comment_block)
        if not header_match:
            return None
