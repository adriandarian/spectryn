...
```bash
# Or via CLI
spectryn sync --parallel --workers 4 --markdown EPIC.md
```
...
spectryn cache clear --project PROJ
```
...
spectryn sync --streaming --markdown huge-epic.md
```
...
```bash
# Explicit batch mode
spectryn sync --batch --batch-size 100 --markdown EPIC.md
```
...
# Profile memory
python -m memory_profiler -m spectryn sync --markdown EPIC.md
```
